...
const PHASE_ID = "PIB_TIKTOK_PHASE_2";
const FEATURE_FLAG = "FF_PIB_TIKTOK_PHASE_2";
const CONNECTOR_ID = "tiktok_ads";
const OUTPUT_CONTRACT_VERSION = "pib_tiktok_phase_2_output_v1";

const FORBIDDEN_FIELDS = ["_debug", "debug_info", "internal_only"];

/**
 * PIB(TikTok) Phase-2: IO Surface Normalization
...
function execute(input) {
    ...
    try {
        span = tracing.startSpan("pib_tiktok_phase_2", {
            phase: PHASE_ID,
        ...
        logStructured("pib_tiktok_phase_2_crash", { execution_id: executionId, error: err.stack });
...
function normalizeIOSurface(contract) {
    ...
    const retryLogic = contract.retry_logic || {}; // Assuming retry_logic is at root or io_schema? Spec says "tiktok_contract.retry_logic" implicitly via "Retry Logic Schema" section, but strict path isn't explicit in input. Usually root.
    // Wait, prompt 3.6 says "Extract deterministic retry_logic.policies". It doesn't explicitly say path in tiktok_contract, but usually it's `contract.retry_logic`. Let's assume root based on usage.
...
function validateInput(input, executionId) {
    ...
    if (!retryLogic || typeof retryLogic !== "object") {
        return buildError(
            input,
            "MISSING_RETRY_LOGIC",
            "Missing or invalid retry_logic in tiktok_contract",
            {},
            executionId
        );
    }
...
function logEvent(status, executionId) {
    logStructured("pib_tiktok_phase_2_event", {
        execution_id: executionId,
        status: status
    });
}
...
function recordMetrics(type, input) {
    if (type === "processed") metrics.count("pib_tiktok_phase_2_processed", 1);
    if (type === "error") metrics.count("pib_tiktok_phase_2_error", 1);
    if (type === "crash") metrics.count("pib_tiktok_phase_2_crash", 1);
    if (type === "disabled") metrics.count("pib_tiktok_phase_2_disabled", 1);
}
