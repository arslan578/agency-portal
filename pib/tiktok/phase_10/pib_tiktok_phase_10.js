...
/**
 * PIB-TIKTOK-PHASE-10: Replay Grounding & Execution Validation
 *
...
// --- CONSTANTS ---
const PHASE_ID = "PIB_TIKTOK_PHASE_10";
const OUTPUT_VERSION = "pib_tiktok_phase_10_output_v1";
...
function validateInput(input) {
    ...
    // Check Feature Flag
    if (input.feature_flags && input.feature_flags.FF_PIB_TIKTOK_PHASE_10 === false) {
        return "NO_OP";
    }
...
function execute(input) {
    ...
    const grounding_snapshot = {
        ...
        envelope_binding: {
            connector_id: envShape.connector_id, // Implicitly validated via envelope plan, but usually connector sets this. We trust input plan but enforce expectation in tests.
            // Actually, for grounding snapshot, we just take what's in envelope shape.
            // But spec requirement says: "Build grounding_snapshot exactly as in the Google code, but with connector_id: 'tiktok_ads' in envelope_binding"
            // Wait. Google code takes `envShape.connector_id`.
            // Does Google code force "google_ads"?
            // Line 173: `connector_id: envShape.connector_id,`
            // So if `envShape` has "tiktok_ads", it works.
            // Strict mirroring implies I keep line 173 as is.
