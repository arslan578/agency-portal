...
const PHASE_ID = "PIB_TIKTOK_PHASE_3";
const FEATURE_FLAG = "FF_PIB_TIKTOK_PHASE_3";
const OUTPUT_CONTRACT_VERSION = "pib_tiktok_phase_3_output_v1";

const FORBIDDEN_FIELDS = ["_debug", "debug_info", "internal_only"];

/**
 * PIB(TikTok) Phase-3: Request Blueprint Generator
...
function execute(input) {
    ...
    try {
        span = tracing.startSpan("pib_tiktok_phase_3", {
            phase: PHASE_ID,
        ...
        logStructured("pib_tiktok_phase_3_crash", { execution_id: executionId, error: err.stack });
...
function logEvent(status, executionId) {
    logStructured("pib_tiktok_phase_3_event", {
        execution_id: executionId,
        status: status
    });
}
...
function recordMetrics(type, input) {
    if (type === "processed") metrics.count("pib_tiktok_phase_3_processed", 1);
    if (type === "error") metrics.count("pib_tiktok_phase_3_error", 1);
    if (type === "crash") metrics.count("pib_tiktok_phase_3_crash", 1);
    if (type === "disabled") metrics.count("pib_tiktok_phase_3_disabled", 1);
}
