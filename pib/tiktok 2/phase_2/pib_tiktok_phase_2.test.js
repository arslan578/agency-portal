const { execute } = require("./pib_tiktok_phase_2");
...
const BASE_CONTRACT = {
    ...
    routing: {
        default_endpoint: "https://ads.tiktok.com/open_api/", // [PLACEHOLDER: Real TikTok endpoints]
...
const TEST_CTX = {
    ...
    phase: "PIB_TIKTOK_PHASE_2",
    feature_flags: { FF_PIB_TIKTOK_PHASE_2: true },
    tenant_context: { tenant_id: "system" },
    tiktok_contract: BASE_CONTRACT
};

describe("PIB-TIKTOK-PHASE-2", () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe("Happy Path", () => {
        ...
        test("4. Routing profile normalization correct", () => {
            const result = execute(TEST_CTX);
            expect(result.io_surface.routing.default_endpoint).toBe("https://ads.tiktok.com/open_api/");
    ...
    describe("Negative Path", () => {
        test("9. Missing tiktok_contract", () => {
        ...
        test("11. Invalid semver", () => {
            const input = { ...TEST_CTX, tiktok_contract: { ...BASE_CONTRACT, version: "v1.0" } };
            const result = execute(input);
            expect(result.status).toBe("ERROR");
            expect(result.errors[0].code).toBe("CONTRACT_VIOLATION");
        });
        ...
        test("12. Wrong connector_id", () => {
            const input = { ...TEST_CTX, tiktok_contract: { ...BASE_CONTRACT, connector_id: "facebook" } };
            const result = execute(input);
            expect(result.status).toBe("ERROR");
            expect(result.errors[0].code).toBe("CONTRACT_VIOLATION");
        });
    ...
    describe("Edge Cases", () => {
        ...
        test("18. Missing routing fields (should handle gracefully but remain deterministic)", () => {
            ...
            // Check stability
            const r2 = execute({ ...TEST_CTX, tiktok_contract: empty });
