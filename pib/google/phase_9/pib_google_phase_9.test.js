...
const phase9 = require("./pib_google_phase_9");
...
const BASE_INPUT = {
    ...
    phase: "PIB_GOOGLE_PHASE_9",
    feature_flags: { "FF_PIB_GOOGLE_PHASE_9": true },
...
describe("PIB Google Phase 9: Safety Horizon Binding", () => {
    ...
    describe("Happy Path", () => {
        test("1. Produces OK status with valid input", () => {
            ...
            expect(result.phase).toBe("PIB_GOOGLE_PHASE_9");
            expect(result.output_contract_version).toBe("pib_google_phase_9_output_v1");
        });
        ...
        test("8. Canonical Hash is stable", () => {
            ...
            i2.feature_flags = { "FF_PIB_GOOGLE_PHASE_9": true, "OTHER": false }; // different key order potential if JS engine varied, but we rely on Phase 9 internal sort
    ...
    describe("Feature Flag", () => {
        test("19. Feature flag OFF -> NO_OP", () => {
            const input = DEEP_COPY(BASE_INPUT);
            input.feature_flags["FF_PIB_GOOGLE_PHASE_9"] = false;
            const result = phase9.execute(input);
