...
const PHASE_ID = "PIB_GOOGLE_PHASE_6";
const FEATURE_FLAG = "FF_PIB_GOOGLE_PHASE_6";
const OUTPUT_CONTRACT_VERSION = "pib_google_phase_6_output_v1";
...
/**
 * PIB(Google) Phase-6: Response Normalizer Mapping
...
function execute(input) {
    ...
    try {
        span = tracing.startSpan("pib_google_phase_6", {
            phase: PHASE_ID,
        ...
        logStructured("pib_google_phase_6_crash", { execution_id: executionId, error: err.stack });
...
function generateResponseNormalizerSpec(ioSurface) {
    ...
    const kaivoErrorCodes = ioSurface.error_mapping && ioSurface.error_mapping.kaivo_error_codes
        ...
        : [];
    const retryOverrides = []; // io_surface doesn't explicitly have retry overrides in Phase 2 error_mapping (it has retry_logic policies), so we start empty or derive?
    // Spec 3.2 says "Build deterministic retry override rules."
    // Phase 2 io_surface has `retry_logic.policies`. 
    // Usually overrides map domain -> policy.
    // Without explicit mapping in input, we keep it empty or deterministic default?
    // "if google domain implies forced retry".
    // We will leave it empty as we don't have the rule logic. The prompt implies we build it, but without data we can't. Empty sorted array is safe.
...
function logEvent(status, executionId) {
    logStructured("pib_google_phase_6_event", {
        execution_id: executionId,
        status: status
    });
}
...
function recordMetrics(type, input) {
    if (type === "processed") metrics.count("pib_google_phase_6_processed", 1);
    if (type === "error") metrics.count("pib_google_phase_6_error", 1);
    if (type === "crash") metrics.count("pib_google_phase_6_crash", 1);
    if (type === "disabled") metrics.count("pib_google_phase_6_disabled", 1);
}
