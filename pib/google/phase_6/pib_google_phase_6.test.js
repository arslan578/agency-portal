const { execute } = require("./pib_google_phase_6");
...
const BASE_INPUT = {
    execution_id: "test-exec-id-p6",
    phase: "PIB_GOOGLE_PHASE_6",
    feature_flags: { FF_PIB_GOOGLE_PHASE_6: true },
    tenant_context: { tenant_id: "system" },
    io_surface: {
        operations: [
            { operation: "CREATE_CAMPAIGN", google_api_method_ref: "c.mutate" },
            { operation: "GET_CAMPAIGN", google_api_method_ref: "c.get" }
        ],
        error_mapping: {
            google_domains: [
                { google_domain: "QUOTA_EXCEEDED", mapped_category: "THROTTLING" },
...
describe("PIB-GOOGLE-PHASE-6", () => {
    ...
    describe("Happy Path", () => {
        ...
        test("4. Deterministic domain -> category mapping", () => {
            ...
            // Based on simple keyword logic in pib_google_phase_6.js
            expect(mapping["QUOTA_EXCEEDED"]).toBe("THROTTLING");
        ...
        test("7. Error plan sorted", () => {
            ...
            // Keys should be sorted in output map (Object keys iteration order usually, but explicit sort in code).
            expect(domains).toEqual(["AUTHENTICATION_ERROR", "INTERNAL_ERROR", "QUOTA_EXCEEDED"]);
        });
