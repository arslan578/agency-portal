const { execute } = require("./pib_google_phase_8");
...
const BASE_INPUT = {
    ...
    phase: "PIB_GOOGLE_PHASE_8",
    feature_flags: { FF_PIB_GOOGLE_PHASE_8: true },
    ...
    routing_profile: { default_endpoint: "https://googleads.googleapis.com" },
...
describe("PIB-GOOGLE-PHASE-8", () => {
    ...
    describe("Happy Path", () => {
        test("1. Returns OK status", () => {
            ...
            expect(result.phase).toBe("PIB_GOOGLE_PHASE_8");
        });

        test("2. Includes correct output contract version", () => {
            const result = execute(DEEP_COPY(BASE_INPUT));
            expect(result.output_contract_version).toBe("pib_google_phase_8_output_v1");
        });
        ...
        test("6. Envelope plan includes canonical envelope_shape bindings", () => {
            ...
            expect(shape.connector_id).toBe("google_ads");
            expect(shape.tenant_id).toBe("t1");
            expect(shape.execution_id).toBe("exec-p8-test");
            expect(shape.phase).toBe("PIB_GOOGLE_PHASE_8");
