...
const PHASE_ID = "PIB_GOOGLE_PHASE_1";
const FEATURE_FLAG = "FF_PIB_GOOGLE_PHASE_1";
const CONNECTOR_ID = "google_ads";
const OUTPUT_CONTRACT_VERSION = "pib_google_phase_1_output_v1";

const FORBIDDEN_FIELDS = ["_debug", "debug_info", "internal_only"];

/**
 * PIB(Google) Phase-1: Connector Contract Ingestion + Capability Surface Registration
...
function execute(input) {
    ...
    try {
        span = tracing.startSpan("pib_google_phase_1", {
            phase: PHASE_ID,
        ...
        logStructured("pib_google_phase_1_crash", { execution_id: executionId, error: err.stack });
...
function logEvent(status, executionId, count) {
    logStructured("pib_google_phase_1_event", {
        execution_id: executionId,
        status: status,
...
function recordMetrics(type, input) {
    if (type === "processed") metrics.count("pib_google_phase_1_processed", 1);
    if (type === "error") metrics.count("pib_google_phase_1_error", 1);
    if (type === "crash") metrics.count("pib_google_phase_1_crash", 1);
    if (type === "disabled") metrics.count("pib_google_phase_1_disabled", 1);
}
