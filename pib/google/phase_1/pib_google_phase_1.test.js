const { execute } = require("./pib_google_phase_1");
...
const TEST_CTX = {
    ...
    feature_flags: { FF_PIB_GOOGLE_PHASE_1: true },
    tenant_context: { tenant_id: "system" },
    google_contract: BASE_CONTRACT
};

describe("PIB-GOOGLE-PHASE-1", () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe("Happy Path", () => {
        test("1. Valid contract returns status OK", () => {
            ...
            expect(result.capability_surface.routing.profile_id).toBe("google_routing_v1");
        });
    ...
    describe("Negative Path", () => {
        test("7. Missing google_contract -> ERROR", () => {
        ...
        test("8. connector_id !== 'google_ads' -> ERROR", () => {
            const input = { ...TEST_CTX, google_contract: { ...BASE_CONTRACT, connector_id: "facebook_ads" } };
            const result = execute(input);
            expect(result.status).toBe("ERROR");
            expect(result.errors[0].code).toBe("CONTRACT_VIOLATION");
        });

        test("9. phase !== 'PIB_GOOGLE_PHASE_1' -> ERROR", () => {
        ...
        test("11. Malformed semver -> ERROR", () => {
            const input = { ...TEST_CTX, google_contract: { ...BASE_CONTRACT, version: "v1" } };
            const result = execute(input);
            expect(result.status).toBe("ERROR");
            expect(result.errors[0].code).toBe("CONTRACT_VIOLATION");
        });
    ...
    describe("Edge Cases", () => {
        ...
        test("14. Unexpected null segments (Handled robustly)", () => {
            ...
            const result = execute({ ...TEST_CTX, google_contract: nulls });
            expect(result.status).toBe("OK");
        ...
        test("18. Canonical hash identical across differently ordered contracts", () => {
            ...
            // Rewrite google_contract with reversed key order
