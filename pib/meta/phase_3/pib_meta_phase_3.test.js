const { execute } = require("./pib_meta_phase_3");
...
const BASE_INPUT = {
    ...
    phase: "PIB_META_PHASE_3",
    feature_flags: { FF_PIB_META_PHASE_3: true },
    ...
    io_surface: {
        operations: [
            ...
            { operation: "GET_CAMPAIGN", google_api_method_ref: "facebook.graph.v19.0.node_id.GET", payload_shape_ref: "meta_ads_get_campaign_payload_v1", idempotency_key_strategy: "NONE" }
        ]
    }
};

describe("PIB-META-PHASE-3", () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe("Happy Path", () => {
        ...
        test("4. Idempotency matrix correct", () => {
            ...
            expect(matrix["CREATE_CAMPAIGN"]).toContain("HASH(");
        ...
        test("6. Payload registry correct and flags set", () => {
            ...
            expect(registry["meta_ads_create_campaign_payload_v1"]).toEqual({ present: true, expansion_allowed: false });
            expect(registry["meta_ads_get_campaign_payload_v1"]).toEqual({ present: true, expansion_allowed: false });
        });

        test("7. Payload registry handles duplicates", () => {
            const dup = JSON.parse(JSON.stringify(BASE_INPUT));
            dup.io_surface.operations.push({ operation: "DUP_OP", google_api_method_ref: "c.mutate", payload_shape_ref: "meta_ads_create_campaign_payload_v1", idempotency_key_strategy: "NONE" });
            const result = execute(dup);
            expect(Object.keys(result.request_blueprint.payload_registry).length).toBe(2);
        });
    ...
    describe("Negative Path", () => {
        ...
        test("16. Feature flag false -> NO_OP", () => {
            const input = { ...BASE_INPUT, feature_flags: { FF_PIB_META_PHASE_3: false } };
            const result = execute(input);
