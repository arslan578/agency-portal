const { execute } = require("./pib_meta_phase_1");
...
const BASE_CONTRACT = {
    ...
    capabilities: {
        ...
        campaign_types: [
            { id: "OUTCOME_SALES", channel: "FACEBOOK", allowed_objectives: ["SALES"], allowed_bidding_strategies: ["COST_CAP"], surfaces: ["FEED"], phase_support: {} },
            { id: "OUTCOME_AWARENESS", channel: "FACEBOOK", allowed_objectives: ["AWARENESS"], allowed_bidding_strategies: ["BID_CAP"], surfaces: ["STORIES"], phase_support: {} }
        ],
        bidding_strategies: [
            { id: "COST_CAP", requires_target_value: true, supported_channels: ["FACEBOOK"] },
            { id: "BID_CAP", requires_target_value: true, supported_channels: ["FACEBOOK"] }
        ],
        targeting: {
            segments: [
                { id: "AGE", required_for_campaign_types: [], supports_negative_targets: false }
            ]
        },
        creative_formats: [
            { id: "IMAGE", channels: ["FACEBOOK"], required_assets: ["IMAGE_HASH"], optional_assets: [] }
        ]
    },
    constraints: {
        budget: { policies: [{ id: "MIN_DAILY_BUDGET" }] },
        categories: { sensitive_categories: ["HOUSING"] },
        regions: {}
    },
    routing: { profile_id: "meta_routing_v1" },
    io_schema: { error_mapping: { id: "meta_errors_v1" } }
};

const TEST_CTX = {
    execution_id: "test-exec-id",
    phase: "PIB_META_PHASE_1",
    feature_flags: { FF_PIB_META_PHASE_1: true },
    tenant_context: { tenant_id: "system" },
    meta_contract: BASE_CONTRACT
};

describe("PIB-META-PHASE-1", () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });
    ...
    describe("Happy Path", () => {
        test("1. Valid contract returns status OK", () => {
            ...
            expect(result.capability_surface.routing.profile_id).toBe("meta_routing_v1");
        });
    ...
    describe("Negative Path", () => {
        test("7. Missing meta_contract -> ERROR", () => {
        ...
        test("8. connector_id !== 'meta_ads' -> ERROR", () => {
        ...
        test("9. phase !== 'PIB_META_PHASE_1' -> ERROR", () => {
        ...
        test("11. Malformed semver -> ERROR", () => {
            const input = { ...TEST_CTX, meta_contract: { ...BASE_CONTRACT, version: "v1" } };
            const result = execute(input);
            expect(result.status).toBe("ERROR");
            expect(result.errors[0].code).toBe("CONTRACT_VIOLATION");
        });
    ...
    describe("Edge Cases", () => {
        ...
        test("14. Unexpected null segments (Handled robustly)", () => {
            ...
            const result = execute({ ...TEST_CTX, meta_contract: nulls });
            expect(result.status).toBe("OK");
        ...
        test("18. Canonical hash identical across differently ordered contracts", () => {
            ...
            // Rewrite meta_contract with reversed key order
