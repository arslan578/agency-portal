...
const PHASE_ID = "PIB_META_PHASE_5";
const FEATURE_FLAG = "FF_PIB_META_PHASE_5";
const OUTPUT_CONTRACT_VERSION = "pib_meta_phase_5_output_v1";

const FORBIDDEN_FIELDS = ["_debug", "debug_info", "internal_only"];

/**
 * PIB(Meta) Phase-5: Routing & Endpoint Binding (Strict Mirror)
...
function execute(input) {
    ...
    try {
        span = tracing.startSpan("pib_meta_phase_5", {
            phase: PHASE_ID,
        ...
        // validator_image is required by spec but not actually used for derivation in Phase 5 logic, same as Google.
        ...
        logStructured("pib_meta_phase_5_crash", { execution_id: executionId, error: err.stack });
...
function validateInput(input, executionId) {
    ...
    // Meta Contract Check (Strict Mirror of Google TP1)
    const contract = input.meta_contract;
    if (!contract || typeof contract !== "object") {
        return buildError(input, "MISSING_CONTRACT", "Missing or invalid meta_contract", {}, executionId);
    }

    if (contract.connector_id !== "meta_ads") {
        return buildError(input, "CONTRACT_VIOLATION", "Invalid connector_id, expected meta_ads", { actual: contract.connector_id }, executionId);
    }
    ...
    if (!contract.retry_logic || typeof contract.retry_logic !== "object") {
        return buildError(input, "MISSING_RETRY_LOGIC", "meta_contract.retry_logic is required", {}, executionId);
    }
...
function logEvent(status, executionId) {
    logStructured("pib_meta_phase_5_event", {
        execution_id: executionId,
        status: status
    });
}
...
function recordMetrics(type, input) {
    if (type === "processed") metrics.count("pib_meta_phase_5_processed", 1);
    if (type === "error") metrics.count("pib_meta_phase_5_error", 1);
    if (type === "crash") metrics.count("pib_meta_phase_5_crash", 1);
    if (type === "disabled") metrics.count("pib_meta_phase_5_disabled", 1);
}
