const { execute } = require("./pib_meta_phase_7");
...
const BASE_INPUT = {
    execution_id: "test-exec-id-p7-meta",
    phase: "PIB_META_PHASE_7",
    feature_flags: { FF_PIB_META_PHASE_7: true },
    tenant_context: { tenant_id: "system" },
    io_surface: {
        error_mapping: {
            google_domains: ["THROTTLED"],
            kaivo_error_codes: [{ code: "THROTTLING" }]
        }
    },
    routing_profile: {
        retry_alignment: {
...
describe("PIB-META-PHASE-7", () => {
    ...
    describe("Happy Path", () => {
        ...
        test("6. No policy match -> default policy id", () => {
            const result = execute(BASE_INPUT);
            const rules = result.error_resolver_spec.resolver_rules.category_to_retry_policy;
            expect(rules["PLATFORM_ERROR"]).toBe("THROTTLING");
        });
    ...
    describe("Negative Path", () => {
        ...
        test("Feature flag false -> NO_OP", () => {
            const input = { ...BASE_INPUT, feature_flags: { FF_PIB_META_PHASE_7: false } };
            const result = execute(input);
            expect(result.status).toBe("NO_OP");
        });
