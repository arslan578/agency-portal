...
const PHASE_ID = "PIB_META_PHASE_8";
const FEATURE_FLAG = "FF_PIB_META_PHASE_8";
const OUTPUT_CONTRACT_VERSION = "pib_meta_phase_8_output_v1";

const FORBIDDEN_FIELDS = ["_debug", "debug_info", "internal_only"];

/**
 * PIB(Meta) Phase-8: Deterministic Recorder Schema & Envelope Plan
 * Produces the canonical recording contract for all Meta connector executions.
 * Pure logic (NO IO).
 */
function execute(input) {
    ...
    try {
        ...
        logStructured("pib_meta_phase_8_event", { phase: PHASE_ID, status: "START" });

        // 1. Validate Input
        const validationError = validateInput(input, input?.execution_id);
        if (validationError) {
            metrics.count("pib_meta_phase_8_error", 1, { code: validationError.errors[0].code });
            if (span) {
        ...
        if (!input.feature_flags[FEATURE_FLAG]) {
            metrics.count("pib_meta_phase_8_disabled", 1);
            logStructured("pib_meta_phase_8_event", { phase: PHASE_ID, status: "NO_OP" });
            if (span) {
        ...
        metrics.count("pib_meta_phase_8_processed", 1);
        logStructured("pib_meta_phase_8_event", { phase: PHASE_ID, status: "SUCCESS", hash: canonicalHash });
        ...
        metrics.count("pib_meta_phase_8_crash", 1);
        logStructured("pib_meta_phase_8_error", { phase: PHASE_ID, error: error.message, stack: error.stack });
        if (span) {
            span.setAttribute("status", "ERROR");
            span.end();
        }
...
function validateInput(input, executionId) {
    ...
    // Single operation flow enforcement as per patch instructions
...
function generateRecorderSchema(input) {
    ...
    // Remove "UNKNOWN" fallback, rely on validateInput
    const operation = request_blueprint.operation;

    // Routing Method Resolution
    ...
    // Response Section Mirrors Phase 6
...
function generateEnvelopePlan(input) {
    ...
    const envelopePlan = {
        envelope_shape: {
            ...
            connector_id: "meta_ads", // Meta specific
            tenant_id: tenantId,
