...
const PHASE_ID = "PIB_META_PHASE_6";
const FEATURE_FLAG = "FF_PIB_META_PHASE_6";
const OUTPUT_CONTRACT_VERSION = "pib_meta_phase_6_output_v1";
...
/**
 * PIB(Meta) Phase-6: Response Normalizer Mapping
...
function execute(input) {
    ...
    try {
        span = tracing.startSpan("pib_meta_phase_6", {
            phase: PHASE_ID,
        ...
        logStructured("pib_meta_phase_6_crash", { execution_id: executionId, error: err.stack });
...
function logEvent(status, executionId) {
    logStructured("pib_meta_phase_6_event", {
        execution_id: executionId,
        status: status
    });
}
...
function recordMetrics(type, input) {
    if (type === "processed") metrics.count("pib_meta_phase_6_processed", 1);
    if (type === "error") metrics.count("pib_meta_phase_6_error", 1);
    if (type === "crash") metrics.count("pib_meta_phase_6_crash", 1);
    if (type === "disabled") metrics.count("pib_meta_phase_6_disabled", 1);
}
