...
const PHASE_ID = "PIB_META_PHASE_2";
const FEATURE_FLAG = "FF_PIB_META_PHASE_2";
const CONNECTOR_ID = "meta_ads";
const OUTPUT_CONTRACT_VERSION = "pib_meta_phase_2_output_v1";

const FORBIDDEN_FIELDS = ["_debug", "debug_info", "internal_only"];

/**
 * PIB(Meta) Phase-2: IO Surface Normalization
...
function execute(input) {
    ...
    try {
        span = tracing.startSpan("pib_meta_phase_2", {
            phase: PHASE_ID,
        ...
        logStructured("pib_meta_phase_2_crash", { execution_id: executionId, error: err.stack });
...
function normalizeIOSurface(contract) {
    ...
    const normsErrorMapping = {
        google_domains: (errorMapping.domains || []).slice().sort((a, b) => a.google_domain.localeCompare(b.google_domain)),
...
function validateInput(input, executionId) {
    ...
    if (!contract || typeof contract !== "object") {
        return buildError(input, "MISSING_FIELD", "Missing or invalid meta_contract", {}, executionId);
    }
    ...
    if (!retryLogic || typeof retryLogic !== "object") {
        return buildError(
            input,
            "MISSING_RETRY_LOGIC",
            "Missing or invalid retry_logic in meta_contract",
            {},
            executionId
        );
    }
...
function logEvent(status, executionId) {
    logStructured("pib_meta_phase_2_event", {
        execution_id: executionId,
        status: status
    });
}
...
function recordMetrics(type, input) {
    if (type === "processed") metrics.count("pib_meta_phase_2_processed", 1);
    if (type === "error") metrics.count("pib_meta_phase_2_error", 1);
    if (type === "crash") metrics.count("pib_meta_phase_2_crash", 1);
    if (type === "disabled") metrics.count("pib_meta_phase_2_disabled", 1);
}
