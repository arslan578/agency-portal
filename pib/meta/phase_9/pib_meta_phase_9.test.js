...
const phase9 = require("./pib_meta_phase_9");
...
const mockP5Routing = {
    ...
    default_endpoint: "https://graph.facebook.com/v18.0" // Meta-specific URL
};
...
const mockP6Normalizer = {
    ...
    error_mapping_plan: {
        google_domain_to_category: { "Google.Rpc.Status": "PLATFORM_ERROR" } // Kept as-is per policy (strict mirror)
    }
};
...
const BASE_INPUT = {
    ...
    phase: "PIB_META_PHASE_9",
    feature_flags: { "FF_PIB_META_PHASE_9": true },
...
describe("PIB Meta Phase 9: Safety Horizon Binding", () => {
    ...
    describe("Happy Path", () => {
        test("1. Produces OK status with valid input", () => {
            ...
            expect(result.phase).toBe("PIB_META_PHASE_9");
            expect(result.output_contract_version).toBe("pib_meta_phase_9_output_v1");
        });
        ...
        test("8. Canonical Hash is stable", () => {
            ...
            i2.feature_flags = { "FF_PIB_META_PHASE_9": true, "OTHER": false }; // different key order potential if JS engine varied, but we rely on Phase 9 internal sort
    ...
    describe("Feature Flag", () => {
        test("19. Feature flag OFF -> NO_OP", () => {
            const input = DEEP_COPY(BASE_INPUT);
            input.feature_flags["FF_PIB_META_PHASE_9"] = false;
            const result = phase9.execute(input);
