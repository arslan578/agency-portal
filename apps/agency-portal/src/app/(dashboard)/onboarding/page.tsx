...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-cream flex flex-col">
      {/* Top Bar */}
      <div className="bg-white border-b-2 border-cream-border px-8 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[16px] font-extrabold text-text-primary">Kaivo Setup</span>
        </div>
        <button onClick={() => router.push('/')} className="text-[12px] font-bold text-text-muted hover:text-text-primary">
          Skip for now →
        </button>
      </div>

      {/* Step Indicator */}
      <div className="bg-white border-b-2 border-cream-border px-8 py-3">
          ...
          {stepLabels.map((label, i) => {
            ...
            return (
              <div key={label} className="flex items-center gap-2 flex-1">
                <div className={`w-[28px] h-[28px] rounded-full flex items-center justify-center text-[11px] font-extrabold shrink-0 ${
                  isDone ? 'bg-teal text-white' : isActive ? 'bg-teal text-white' : 'bg-cream-dark text-text-muted'
                }`}>
                  {isDone ? '✓' : s}
                </div>
                <span className={`text-[12px] font-bold whitespace-nowrap ${isActive ? 'text-text-primary' : 'text-text-muted'}`}>{label}</span>
                {i < 3 && <div className={`flex-1 h-[2px] mx-2 rounded-full ${step > s ? 'bg-teal' : 'bg-cream-dark'}`} />}
              </div>
            );
          })}
        </div>
        ...
        <div className="w-full max-w-[640px]">
          {step === 1 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-[24px] font-extrabold text-text-primary">Set up your agency</h2>
                <p className="text-[14px] text-text-muted mt-1">Tell us about your agency to personalize your experience.</p>
              </div>

              <div className="bg-white rounded-xl border-2 border-cream-border p-6 space-y-5">
                <div className="flex items-center gap-5">
                  <div className="w-[64px] h-[64px] rounded-full bg-cream border-2 border-cream-border flex items-center justify-center text-text-muted text-[20px] shrink-0 cursor-pointer hover:bg-cream-dark transition-colors">
                    📷
                  </div>
                  <div className="flex-1">
                    <div className="text-[12.5px] font-bold text-text-primary mb-[6px]">Agency Logo</div>
                    <div className="text-[11.5px] text-text-muted">Click to upload (PNG, JPG, max 2MB)</div>
                  </div>
                </div>

                <div>
                  <label className="block text-[12.5px] font-bold text-text-primary mb-[6px]">Agency Name *</label>
                  <input
                    type="text"
                    value={agencyName}
                    onChange={e => setAgencyName(e.target.value)}
                    placeholder="Your Agency Name"
                    className="w-full h-[44px] px-3 border-2 border-cream-border rounded-[10px] bg-cream text-[13.5px] text-text-primary placeholder:text-text-muted focus:outline-none focus:border-teal focus:ring-1 focus:ring-teal"
                  />
                </div>

                <div>
                  <label className="block text-[12.5px] font-bold text-text-primary mb-[6px]">Contact Email</label>
                  <input
                    type="email"
                    value={agencyEmail}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[12.5px] font-bold text-text-primary mb-[6px]">Timezone</label>
                    <select
                      value={timezone}
                      onChange={e => setTimezone(e.target.value)}
                      className="w-full h-[44px] px-3 border-2 border-cream-border rounded-[10px] bg-cream text-[13.5px] text-text-primary focus:outline-none focus:border-teal"
                    >
                      {TIMEZONES.map(tz => <option key={tz} value={tz}>{tz}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-[12.5px] font-bold text-text-primary mb-[6px]">Currency</label>
                    <select
                      value={currency}
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="bg-teal hover:bg-teal-dark text-white font-bold text-[13.5px] h-[44px] px-8 rounded-[10px] transition-colors"
                >
                  Continue →
                </button>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-[24px] font-extrabold text-text-primary">Connect your platforms</h2>
              ...
              <div className="grid grid-cols-2 gap-4">
                {PLATFORMS.map(p => {
                  ...
                  return (
                    <div key={p.id} className={`bg-white rounded-xl border-2 p-5 transition-colors ${isConnected ? 'border-teal' : 'border-cream-border'}`}>
                      <div className="flex items-center gap-3 mb-4">
                        <div
                          className="w-[40px] h-[40px] rounded-lg flex items-center justify-center text-[15px] font-bold shrink-0"
                          style={{ background: p.bg, color: p.color }}
                        >
                          {p.icon}
                        </div>
                        <div>
                          <div className="text-[13px] font-bold text-text-primary">{p.name}</div>
                          {isConnected && (
                            <span className="text-[10.5px] font-bold text-green">● Connected</span>
                          )}
                        </div>
                      </div>
                      {isConnected ? (
                        <button
                          ...
                          className="w-full h-[36px] border-2 border-cream-border rounded-lg text-[12px] font-bold text-text-muted hover:border-coral hover:text-coral transition-colors"
                        >
                          Disconnect
                        </button>
                      ) : (
                        <button
                          ...
                          className="w-full h-[36px] bg-teal text-white rounded-lg text-[12px] font-bold hover:bg-teal-dark transition-colors disabled:opacity-60"
                        >
                          {isConnecting ? (
              ...
              <div className="flex justify-between">
                <button onClick={() => setStep(1)} className="text-[13px] font-bold text-text-muted hover:text-text-primary">
                  ← Back
                </button>
                <button
                  onClick={() => setStep(3)}
                  disabled={connectedPlatforms.size === 0}
                  className="bg-teal hover:bg-teal-dark text-white font-bold text-[13.5px] h-[44px] px-8 rounded-[10px] transition-colors disabled:opacity-50"
                >
                  Continue →
                </button>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              <div>
                <h2 className="text-[24px] font-extrabold text-text-primary">Select ad accounts</h2>
              ...
              <div className="bg-white rounded-xl border-2 border-cream-border overflow-hidden">
                ...
                {availableAccounts.length === 0 ? (
                  ...
                  <div className="divide-y divide-cream-border/50">
                    {availableAccounts.map(acc => {
                      const isSelected = selectedAccounts.has(acc.id);
                      const plat = PLATFORMS.find(p => p.id === acc.platform);
                      return (
                        <label key={acc.id} className="flex items-center gap-4 px-5 py-3 hover:bg-cream/50 cursor-pointer transition-colors">
                          <input
                            type="checkbox"
                            checked={isSelected}
                            onChange={() => toggleAccount(acc.id)}
                            className="w-4 h-4 rounded border-cream-border text-teal focus:ring-teal accent-[#2a9d8f]"
                          />
                          <div
                            className="w-[28px] h-[28px] rounded-md flex items-center justify-center text-[11px] font-bold shrink-0"
                            style={{ background: plat?.bg, color: plat?.color }}
                          >
                            {plat?.icon}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-[12.5px] font-bold text-text-primary truncate">{acc.name}</div>
                            <div className="text-[11px] text-text-muted">{plat?.name}</div>
                          </div>
                          <span className="text-[11.5px] font-bold text-text-muted font-mono">{acc.spend}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div>
                <label className="block text-[12.5px] font-bold text-text-primary mb-[6px]">Historical Data Window</label>
                <div className="flex gap-2">
                  {(['30d', '90d', '6mo', '1yr'] as const).map(w => (
                    <button
                      key={w}
                      onClick={() => setDataWindow(w)}
                      className={`px-4 h-[36px] rounded-lg text-[12px] font-bold transition-colors ${
                        dataWindow === w ? 'bg-teal text-white' : 'bg-cream text-text-muted border border-cream-border hover:bg-cream-dark'
                      }`}
                    >
                      {w}
                    </button>
              ...
              <div className="flex justify-between">
                <button onClick={() => setStep(2)} className="text-[13px] font-bold text-text-muted hover:text-text-primary">
                  ← Back
                </button>
                <button
                  onClick={startSync}
          ...
          {step === 4 && (
            <div className="space-y-6">
              {!syncComplete ? (
                <>
                  <div>
                    <h2 className="text-[24px] font-extrabold text-text-primary">Syncing your data...</h2>
                    <p className="text-[14px] text-text-muted mt-1">Importing historical performance data. This usually takes about a minute.</p>
                  </div>

                  <div className="bg-white rounded-xl border-2 border-cream-border p-5 space-y-4">
                        ...
                        <div key={accId} className="space-y-2">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              <div
                                className="w-[22px] h-[22px] rounded-md flex items-center justify-center text-[9px] font-bold"
                                style={{ background: plat?.bg, color: plat?.color }}
                              >
                                {plat?.icon}
                              </div>
                              <span className="text-[12.5px] font-bold text-text-primary">{acc?.name}</span>
                            </div>
                            <span className="text-[11px] font-bold text-text-muted font-mono">{prog}%</span>
                          </div>
                          <div className="w-full h-[6px] bg-cream-dark rounded-full overflow-hidden">
                            <div
                ...
                <div className="text-center py-12 space-y-6">
                  <div className="w-[72px] h-[72px] rounded-full bg-teal-light flex items-center justify-center mx-auto">
                    <svg className="w-8 h-8 text-teal" fill="none" stroke="currentColor" strokeWidth="3" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                  </div>
                  <div>
                    <h2 className="text-[28px] font-extrabold text-text-primary">You&apos;re all set!</h2>
                  ...
                  <div className="flex gap-6 justify-center text-center">
                    <div className="bg-white rounded-xl border-2 border-cream-border px-6 py-4">
                      <div className="text-[24px] font-extrabold text-teal font-mono">{connectedPlatforms.size}</div>
                      <div className="text-[11px] font-bold text-text-muted mt-1">Platforms</div>
                    </div>
                    <div className="bg-white rounded-xl border-2 border-cream-border px-6 py-4">
                      <div className="text-[24px] font-extrabold text-teal font-mono">{selectedAccounts.size}</div>
                      <div className="text-[11px] font-bold text-text-muted mt-1">Accounts</div>
                    </div>
                    <div className="bg-white rounded-xl border-2 border-cream-border px-6 py-4">
                      <div className="text-[24px] font-extrabold text-teal font-mono">{dataWindow}</div>
                      <div className="text-[11px] font-bold text-text-muted mt-1">History</div>
                    </div>
                  </div>
                  <button
                    onClick={handleFinish}
                    className="bg-teal hover:bg-teal-dark text-white font-bold text-[14px] h-[48px] px-10 rounded-[10px] transition-colors"
                  >
                    Go to Dashboard →
                  </button>
                </div>
