...
export function ClientMetaSection({ clientId }: { clientId: number }) {
  ...
  if (loading) {
    return (
      <section className="bg-white rounded-xl border-2 border-cream-border p-5 animate-pulse">
    ...
    return (
      <section className="bg-white rounded-xl border-2 border-cream-border overflow-hidden">
        <div className="px-5 py-4 border-b-2 border-cream-border bg-cream-dark/30 flex items-center justify-between">
          <div>
            <h2 className="text-[14px] font-extrabold text-[#1877f2] flex items-center gap-2">
    ...
    return (
      <section className="bg-white rounded-xl border-2 border-red/20 overflow-hidden">
        <div className="bg-red-light/50 p-5">
          <h2 className="text-[14px] font-extrabold text-red flex items-center gap-2">
  ...
  return (
    <section className="bg-white rounded-xl border-2 border-cream-border overflow-hidden">
      <div className="px-5 py-4 border-b-2 border-cream-border bg-[#e8effe]/30 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-[14px] font-extrabold text-[#1877f2] flex items-center gap-2">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2C6.477 2 2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.879V14.89h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.989C18.343 21.129 22 16.99 22 12c0-5.523-4.477-10-10-10z" />
          ...
          {insights.ad_accounts?.[0] && (
            <p className="text-[12px] text-text-muted font-medium mt-0.5">
              Account: <span className="text-text-primary font-bold">{insights.ad_accounts[0].account_name}</span>
        ...
        <button
          ...
          disabled={refreshing}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-bold border-2 border-[#1877f2]/20 bg-white text-[#1877f2] hover:bg-[#1877f2]/10 transition-colors disabled:opacity-50"
        >
          {refreshing ? (
      ...
      <div className="p-5">
        <h3 className="text-[11px] font-bold text-text-muted uppercase tracking-wide mb-3">Live Campaigns</h3>
        {insights.campaigns.length === 0 ? (
          <div className="rounded-lg bg-cream/40 border-2 border-cream-border p-4 text-center">
             <p className="text-[13px] text-text-muted font-medium">No campaigns found in this Meta ad account.</p>
          </div>
        ) : (
          <div className="overflow-x-auto rounded-lg border-2 border-cream-border">
            <table className="w-full text-left text-[13px]">
              <thead>
                <tr className="border-b-2 border-cream-border bg-cream/40">
                  <th className="px-4 py-2 font-bold text-text-muted text-[11px] uppercase">Name</th>
                  <th className="px-4 py-2 font-bold text-text-muted text-[11px] uppercase">Status</th>
                  <th className="px-4 py-2 font-bold text-text-muted text-[11px] uppercase text-right">Spend</th>
                  <th className="px-4 py-2 font-bold text-text-muted text-[11px] uppercase text-right">Impressions</th>
                  <th className="px-4 py-2 font-bold text-text-muted text-[11px] uppercase text-right">Clicks</th>
                </tr>
              </thead>
              <tbody>
                {insights.campaigns.map((c) => (
                  <tr key={c.campaign_id} className="border-b border-cream-border last:border-b-0 hover:bg-cream-dark/20">
                    <td className="px-4 py-2.5 font-bold text-text-primary">{c.name}</td>
                    <td className="px-4 py-2.5">
                      <span className={`inline-flex px-2 py-0.5 rounded-md text-[10.5px] font-bold ${
                        c.status === 'ACTIVE' ? 'bg-green-light text-green' : 'bg-cream text-text-muted'
                      }`}>
                        {c.status}
                      </span>
                    </td>
