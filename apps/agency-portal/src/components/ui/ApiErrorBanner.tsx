  ...
  return (
    <div className="rounded-xl border-2 border-red/20 bg-red-light p-4 flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-bold text-red">{title ?? 'Error loading data'}</p>
      ...
      {onRetry && (
        <button
          type="button"
          onClick={onRetry}
          className="shrink-0 px-3 py-1.5 rounded-lg border-2 border-red/30 bg-white text-[12px] font-bold text-red hover:bg-red-light transition-colors"
        >
          Retry
        </button>
      )}
