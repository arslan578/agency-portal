...
function SpotifyOAuthCallbackContent() {
    ...
    useEffect(() => {
        const handleCallback = async () => {
            try {
                ...
                const callbackUrl = `${API_ENDPOINTS.PLATFORM.SPOTIFY.OAUTH.CALLBACK}?code=${encodeURIComponent(
                    code
                )}&state=${encodeURIComponent(state)}`;

                console.log('[Spotify OAuth] Calling backend callback with state:', state);
                ...
                console.log('[Spotify OAuth] Backend callback response:', response);

                if (response.success) {
                    console.log('[Spotify OAuth] SUCCESS - Account connected! account_id:', response.account_id);
                    setStatus('success');
                    setMessage(response.message || 'Spotify account connected successfully!');
                    ...
                    setStatus('error');
                    setMessage(response.error || response.message || 'Failed to connect Spotify account');
                }
            } catch (err: any) {
            ...
            <div className="max-w-md w-full bg-kaivo-card-bg rounded-lg shadow-lg p-8 text-center">
                {status === 'loading' && (
                    <>
                        ...
                        <h2 className="text-xl font-semibold text-white mb-2">Connecting Spotify Account...</h2>
