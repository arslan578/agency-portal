    ...
    useEffect(() => {
        const handleCallback = async () => {
            try {
                ...
                const callbackUrl = `${API_ENDPOINTS.PLATFORM.TIKTOK.OAUTH.CALLBACK}?code=${encodeURIComponent(
                    code
                ...
                if (response.success) {
                    setStatus('success');
                    setMessage(response.message || 'TikTok account connected successfully!');
                    ...
                    setStatus('error');
                    setMessage(response.error || response.message || 'Failed to connect TikTok account');
                }
            } catch (err: any) {
            ...
            <div className="max-w-md w-full bg-kaivo-card-bg rounded-lg shadow-lg p-8 text-center">
                {status === 'loading' && (
                    <>
                        ...
                        <h2 className="text-xl font-semibold text-white mb-2">Connecting TikTok Account...</h2>
