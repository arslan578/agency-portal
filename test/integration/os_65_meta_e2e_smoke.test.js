...
const CAPABILITY = { caps: "A_Meta" };
const IO = { io: "B_Meta" };
const POLICY = { pol: "C_Meta" };
const SAFETY = { safe: "D_Meta" };
const REPLAY = { rep: "E_Meta" };
const READY = { ready: "F_Meta", version: "1.0.0", connector_id: "meta_ads" };
const CHECK = { check: "G_Meta" };

// Meta uses 12 phases
const PIB_HASH_CHAIN = {};
for (let i = 1; i <= 12; i++) {
    PIB_HASH_CHAIN[String(i)] = { canonical_hash: `meta_hash${i}` };
}
...
describe("OS-65 Meta E2E Smoke Test", () => {
    test("PIB-12-style packet -> OS-65 decoder -> connector_registry.meta_ads", () => {
        ...
        // Basic success
        if (result.status !== "OK") {
            console.error("Meta OS-65 Error:", JSON.stringify(result, null, 2));
        }
        ...
        expect(result.connector_registry.meta_ads).toBeDefined();

        const entry = result.connector_registry.meta_ads;

        // Identity + version
        expect(entry.connector_id).toBe("meta_ads");
        expect(entry.version).toBe("1.0.0");

        // Canonical hash present and well-formed
        expect(entry.canonical_hash).toMatch(/^[a-f0-9]{64}$/);

        // Hash chain propagated, including phase 12
        expect(entry.pib_hash_chain).toBeDefined();
        expect(entry.pib_hash_chain["12"]).toBeDefined();
        expect(typeof entry.pib_hash_chain["12"].canonical_hash).toBe("string");
